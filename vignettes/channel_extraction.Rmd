...
Subsequent analysis steps of riverMapper require a channel network which is commonly derived from a digital elevation model. riverMapper has a wrapper for SAGA GIS as well as to GRASS GIS to conduct channel network extraction from a digital elevation model. Both GIS backends have their own pros and cons, but in general the preference should be given to GRASS GIS when it comes to large scale river system analysis with globally available open access elevation models. 
...
#Specfify the path to your SAGA; the function rsaga.env() from the RSAGA package allows to search for the path on your drive
